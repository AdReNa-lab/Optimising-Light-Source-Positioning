function [Illumination_Data_Reduced] = Convhull_Option_Reduction(Illumination_Data, Std_Dev_Selector, Total_Flux_Tolerance, Standard_Deviation_Tolerance)

%This function uses a modified convex hull method to select those points which
%maximize total flux for a given standard deviation. 
...
%deviation. Therefore, any points that correspond to a high standard deviation
%and low total intensity can be disregarded.  As the convhull() function
%chooses points to encompass the entire range of the data, there will be
%some points that represent a high standard deviation and low intensity.
%The following lines of code identify these and removes them from the data
%set.  
[~, idx_upper] = max(KI);
...
for i = 1:length(Ks)-1
    ...
    linear_fit_x = (intensity_subset-p(2))./p(1);
...
Illumination_Data_Reduced.standard_deviation_percentage = data_Std_Perc(Index_unique);
...
dim = [0.625 0.15 .1 .1];
str = ['Initial Number of Options:  ', num2str(length(Illumination_Data.total_flux))];
